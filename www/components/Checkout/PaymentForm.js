...
mutation($orderId: ID!, $paypalOrderId: ID!, $shipping: Float!, $promo: String) {
  placeOrder(orderId: $orderId, paypalOrderId: $paypalOrderId, shipping: $shipping, promo: $promo) {
    id
  }
}
`
...
export default function PaymentForm({ cartData, orderId, shippingAddress, shippingType, promo, handleBack }) {
  // TODO: implement this
  return (
    <Mutation mutation={placeCartOrder} variables={{ orderId: orderId, shipping: Number.parseFloat(shippingType) || 0.00, promo, }}>
      {(mutationFn, { loading, error, data }) =>
        loading ? <Loader />
        : error ? <ApolloError error={error} />
        : data ? <p>Order placed</p>
        : <React.Fragment>
          <Typography variant="h4" gutterBottom>
            Payment method
          </Typography>
          <Typography variant="subtitle2" gutterBottom>
            By placing an order you agree to the <Link href="/ShippingInfo/shipping">
            <a target="_blank">
          ...
          <Grid container spacing={3}>
               ...
               createOrder={(data, actions) => {
                 ...
                 return actions.order.create({
                   purchase_units: [{
                     ...
                     items: cartData.items.map(({ product, qty, price }) => {
                       return {
                         sku: product.sku,
                         name: product.name,
           ...
           <Grid item xs={12}>
            <CheckoutNavButtons handleBack={handleBack} handleNext={() => {}} canAdvance={false} />
           </Grid>
         </Grid>
       </React.Fragment>
        }
