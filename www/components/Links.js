...
export const ProductLinkStr = props =>
  `/product/${props.productId}/${slugify("" + props.category)}/${slugify(
    "" + props.subcategory
  )}/${slugify("" + props.title)}?listref=${props.listName}`;

export const ProductLink = props => (
  <Link
    href={`/product?productId=${props.productId}&listref=${props.listName}`}
    as={ProductLinkStr(props)}
    prefetch
  >
    <a
      onClick={() =>
...
export const SearchLinkStr = originalArgs => {
