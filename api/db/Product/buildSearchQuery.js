...
const WherePrice = (query, operator, value) => {
  return query.where(builder =>
    builder
      .orWhere(builder =>
        builder.where("Price1", ">", 0).where("Price1", operator, value)
      )
      .orWhere(builder =>
        builder
          .where("SalePrice", ">", 0)
      ...
      .orWhere(builder => builder.where("ItemPrice", operator, value))
  );
};
...
const buildSearchQuery = (
  ...
  const makeQueryWithSuffix = suffix => {
    const doSearch =
      searchPhrase && searchEngine.tokenizerFn(searchPhrase).length > 0;
    let q = builder
      .select([
        "Products.ID as ID",
    ...
    if (doSearch) {
      const matchSku = builder
        .select("Products.ID as id", knex.raw("9999 as relevance"))
        .from("Products")
        .where("ItemID", searchPhrase);

      q = q.innerJoin(
        matchSku
          .union(searchEngine.search(searchPhrase))
          .as("_Search" + suffix),
        "_Search" + suffix + ".id",
        "Products.ID"
      );
