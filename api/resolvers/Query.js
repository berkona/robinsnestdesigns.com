const { verifyAuthTokenAsync, getUserFromToken } = require("../auth");
...
module.exports = {
  siteinfo: (obj, args, context) => {
    return {};
  },
  category: (obj, args, context) =>
    getCategory(args.categoryId).then(x => reduceCategory(x[0])),
  allCategories: (obj, args, context) =>
    listCategories().then(reduceAllCategories),
  allSubcategories: (obj, args, context) =>
    listSubcategories(args.categoryId).then(reduceAllCategories),
  product: (obj, args, context) =>
    getProduct(args.productId).then(x => {
      if (!x || x.length == 0)
  ...
  cart: (obj, args, context) =>
    getOrder(args.orderId, args.shipping, args.county, args.promo),
  ...
  relatedProducts: async (obj, { productId }, context) =>
    await relatedProducts(productId),
  topSellingProducts: async (obj, { limit }, context) =>
    await topSellingProducts(null, limit)
